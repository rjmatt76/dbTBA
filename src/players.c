...
int load_char(const char *name, struct char_data *ch)
{
  ...
  if(USING_MYSQL_DATABASE_FOR_PLAYERFILE == TRUE) {
    return load_char_mysql(name, ch);
  }
...
*/
	...
	break;
      ...
      case 'G':
//       if (!strcmp(tag, "God "))  GET_GOD(ch) = atoi(line);
      ...
      case 'K':
       if(!strcmp(tag, "Kamt")) {
          ...
          do {
            ...
            sscanf(line, "%d %d", &j, &k);
            if(j != -1)
              kill_add(ch, j, k, TRUE);
            i++;
          } while ((j != -1) && (i != MAX_KILL_MEMORY)
           && (k != 0) && (j != MAX_KILL_MEMORY));
       }
...
*/
  ...
  break;
      ...
      /* strife in format 0:0:0:0 -Cyric */
	    sprintarray_conv(PRF_FLAGS(ch), f1, PR_ARRAY_MAX);
	  }
        break;
...
//              if(k == 0) {
//                GET_SPELLMEM(ch, GET_MEMCURSOR(ch)) = j;
//                GET_MEMCURSOR(ch)++;
...
//  if(GET_GOD(ch)       != PFDEF_GOD)      fprintf(fl, "God : %d\n", GET_GOD(ch));
...
/*
  ...
  fprintf(fl, "-1 0\n");
*/
  /* Cooldown timers (replaces/includes innate timers) 
     but timer is pulse based instead of tick based */
/*
  fprintf(fl, "Cool:\n");
  for (cd = ch->cooldown; cd; cd = next_cd) {
    next_cd = cd->next;
    fprintf(fl, "%d %d\n", cd->spellnum, cd->timer);
  }
  fprintf(fl, "-1 0\n");
  ...
  for(i = 0; i <= NUM_FEAT_TYPES; i++) {
    if(GET_FEAT_SLOTS(ch, i))
      fprintf(fl, "%d %d\n", i, GET_FEAT_SLOTS(ch, i));
    }
  fprintf(fl, "-1 -1\n");
*/
//  fprintf(fl, "Spk : %d\n", SPEAKING(ch));
...
void clean_pfiles(void)
{
  ...
  for (i = 0; i <= top_of_p_table; i++) {
	...
	//remove_player(i);
        remove_player_mysql(player_table[i].id);
      } else {
...
/* returns the format 12345:12345:12345:12345
   ...
   maxar+1 times -Cyric
*/
char* sprintarray_write(int bitvector[], int maxar)
{
  int i;
  char array_buf[MAX_STRING_LENGTH];
  char array_buf2[MAX_STRING_LENGTH];

  *array_buf = '\0';

  for(i = 0; i < maxar; i++)
  {
    sprintf(array_buf2, "%d:", bitvector[i]);
    strcat(array_buf, array_buf2);
  }
  return strdup(array_buf);
}
